    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50 overflow-y-auto">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-4xl w-full transform transition-all animate-fade-in-down border border-gray-200 dark:border-gray-700 max-h-[90vh] flex flex-col">
                        ...
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                {(proforma.detalles || []).map((detalle) => {
                                    ...
                                    if (detalle.piezas) {
                                        allPieces = detalle.piezas.split('/').map(p => p.trim());
                                        historia.forEach(h => {
                                            if (h.proformaDetalleId === detalle.id && h.estadoTratamiento === 'terminado' && h.pieza) {
                                                const hPieces = h.pieza.split('/').map(p => p.trim());
                ...
                <div className="p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50 rounded-b-xl flex justify-end">
                    <button
                        onClick={onClose}
                        className="px-6 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-lg font-bold shadow-md transition-all transform hover:-translate-y-0.5"
                    >
                        Cerrar
                    </button>
                </div>
