                            ...
                            ${allFormasPago.map((fp: FormaPago, index: number) => `
                                <tr>
                                    ...
                                    <td class="${fp.estado === 'activo' ? 'status-active' : 'status-inactive'}">
                                        ${fp.estado.charAt(0).toUpperCase() + fp.estado.slice(1)}
                                    </td>
                                </tr>
                            `).join('')}
                        </tbody>
    ...
    return (
        <div className="content-card p-6">
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                <h2 className="text-2xl font-bold text-gray-800 dark:text-white">Formas de Pago</h2>
                <div className="flex gap-2">
                    ...
                    <button onClick={handlePrint} className="bg-purple-600 hover:bg-purple-700 text-white font-semibold py-2 px-4 rounded-lg flex items-center gap-2 shadow-md transition-all transform hover:-translate-y-0.5">
            ...
            <div className="mb-2 text-sm text-gray-600 dark:text-gray-400">
                Mostrando {formasPago.length} de {total} resultados
            </div>
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        {formasPago.map((fp, index) => (
                            <tr key={fp.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-200">{fp.forma_pago}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm">
                                    <span className={`px-2 py-1 rounded text-sm ${fp.estado === 'activo' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200'}`}>
                                        {fp.estado}
                                    </span>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm flex gap-2">
                                    <Link
                                        to={`/forma-pago/edit/${fp.id}`}
